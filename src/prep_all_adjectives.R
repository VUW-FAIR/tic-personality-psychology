...
allCharFiles <- list.files("resources/supervised/Character Lists/")
allCharFiles <- allCharFiles[which(allCharFiles!="Icon\r")]
allTextFiles <- list.files("resources/supervised/Text Files")
allTextFiles <- allTextFiles[which(allTextFiles!="Icon\r")]

#for(sliceSize in list("sentence")){
#for(sliceSize in list(1000)){
for(sliceSize in list(1000,"sentence",100,200,400,600)){
  ...
  for(nextRun in 1:length(allTextFiles)){
    ...
    print(paste0('resources/supervised/Text Files/',allTextFiles[nextRun]))
    print(paste0('resources/supervised/Character Lists/',allCharFiles[nextRun]))
    
    sourceText <- readChar(paste0('resources/supervised/Text Files/',allTextFiles[nextRun]),
                           file.info(paste0('resources/supervised/Text Files/',allTextFiles[nextRun]))$size)
    processedText <- gsub("\\r\\n", " ", sourceText, perl = T)
    ...
    outer_lapply <- lapply(words300B, function(xx){
      result = tryCatch({
        ...
        if(personCentric){
          ...
          neg_deps <- data.frame(token=character(0),sentence_id=numeric(0),stringsAsFactors = F)
          for(ne in 1:nrow(negs)){
            neg_deps <- rbind(neg_deps, data.frame(token=annotation[which(annotation$sentence_id==negs[ne,2] & annotation$token_id==negs[ne,8]),4],sentence_id=annotation[which(annotation$sentence_id==negs[ne,2] & annotation$token_id==negs[ne,8]),2],stringsAsFactors = F))
          }
          ...
          advs <- annotation[annotation$pos == "ADJ",c("lemma","token","sentence_id")]
          
          #character list
          tmp <- readLines(paste0('resources/supervised/Character Lists/',allCharFiles[nextRun]))
          ...
          for(k in 1:nrow(chars)){
            ...
            matched <- unlist(gregexpr(paste("[\\s\\\\\"](",needle,")[\\';,.:\\s\\\\\"]",sep=""), xx,perl=TRUE))
            multiple <- F
            if(k+1<=nrow(chars)){
            ...
            if(!multiple){
              ...
              real_xx <- xx
              xx <- gsub(needle,paste(replicate(nWords, "FOOBAR"), collapse = " "),xx)
            }
            
            if(matched != -1){
              peplist[[length(peplist)+1]]<-chars[k,1]
            }
          }
          ...
          NLP_text <- NLP::as.String(real_xx)
          
          a2 <- NLP::annotate(NLP_text, list(sent_token_annotator, word_token_annotator))
          p <- parse_annotator(NLP_text, a2)
          ...
          if(stemTrait){
            ...
            xx_sentence <- xx_sentence[which(!is.na(matched))]
            if(!allwords) {traitwords_out_lem <- xx_lem[which(!is.na(matched))]}
          }
          
          traitwords_out <- data.frame(token=traitwords_out,sentence_id=xx_sentence,stringsAsFactors = F)
          ...
          traitwords_out[which(traitwords_out$token %in% neg_deps$token & traitwords_out$sentence_id %in% neg_deps$sentence_id),1] <- tolower(paste0("#-",traitwords_out[which(traitwords_out$token %in% neg_deps$token & traitwords_out$sentence_id %in% neg_deps$sentence_id),1]))
          
          if(length(traitwords_out)>0 & length(peplist)>0){ 
            traitwords_out <- unlist(lapply(traitwords_out$token,function(x){tolower(paste0(peplist,"::",x))}))
          } else {
            traitwords_out <- c()
          }
          ...
          xx_lem <- advs$lemma
          xx_sentence <- advs$sentence_id
          traitwords_out <- data.frame(token=xx,sentence_id=xx_sentence,stringsAsFactors = F)
          
          ## Replacing lemmatised word with token word to allow for matching of negative dependencies
          #traitwords_out[which(traitwords_out == tokens$lemma[which(traitwords_out == tokens$lemma)])] <- tolower(tokens$token[which(tokens$lemma == traitwords_out)])
          traitwords_out[which(traitwords_out$token %in% neg_deps$token & traitwords_out$sentence_id %in% neg_deps$sentence_id),1] <- tolower(paste0("#-",traitwords_out[which(traitwords_out$token %in% neg_deps$token & traitwords_out$sentence_id %in% neg_deps$sentence_id),1]))
          traitwords_out <- traitwords_out$token
        }
      ...
      }, error = function(error_condition) {print(error_condition)})
    })
